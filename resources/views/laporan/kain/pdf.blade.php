											...
											@foreach ($kainReport as $item)
												<tr class="align-top">
													...
													<td>
														...
														@if ($item->kode_desain)
															<br>Desain: <strong>{{ $item->kode_desain }} </strong>
														@endif
														@if ($item->lot)
														...
														@else
														@endif
														<span class="badge bg-warning">{{ $item->supplier->nama_supplier }}</span>
														<br>
														@php
															$totalPcsKain = 0;
															$totalYardKain = 0;
														...
														@foreach ($item->warnas as $w)
															...
															@endphp
														@endforeach
														<strong>Pcs Total: {{ $totalPcsKain }}</strong>
													</td>
													{{-- <td>
														<div class="bow">
															...
															@foreach ($item->warnas->sortBy('nama_warna') as $warna)
																<div class="bol-6">
																	<strong>{{ $warna->nama_warna }}:</strong> || {{ $warna->total_pcs }} Pcs
																</div>
																@php $counter++; @endphp
																@if ($counter % 2 == 0)
																@endif
																...
																<tbody>
																	@foreach ($item->warnas->sortBy('nama_warna') as $warna)
																		<tr>
																			...
																			<td>
																				<strong>{{ $warna->total_pcs }} Pcs</strong>
																			</td>
																		</tr>
																	@endforeach
																</tbody>
